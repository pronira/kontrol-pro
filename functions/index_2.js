/**
 * GrantFlow ScanEngine v6
 * ─────────────────────────────────────────────────────────────
 * ВІДНОВЛЕНО (втрачено в v5→нова версія):
 *  - fingerprint-дедуп (перше речення + URL + заголовок)
 *  - isWithinWindow() — фільтр за датою публікації
 *  - extractMultipleGrants() — кілька грантів на одній сторінці
 *  - splitTelegramPost() — розбивка довгих постів
 *  - diagLog + diag.steps/warnings у scanSingle
 *  - збереження лога в gf_scan_logs + last_scan_log на джерелі
 *  - HTTP endpoints: scanSource, rejectDetected, scanAll
 *  - scanScheduled: перевірка інтервалу + 10 кандидатів
 *
 * ЗБЕРЕЖЕНО ПОКРАЩЕННЯ нової версії:
 *  - item_limit з налаштувань джерела (дефолт 10, макс 30)
 *  - passesFilter: source_keywords
 *  - extractDeadline: більше паттернів
 *  - extractAmount: розширені паттерни
 *  - classify: більше донорів/тематик/заявників
 *  - fetchDetailPage: AbortController + більше селекторів + 6000 символів
 *  - parseRSS: Atom 1.0, dc:date, очищення Google News
 *  - parseTelegram: всі зовнішні URL
 *  - parsePageLinks: семантичні теги article/h2/h3, нормалізація URL
 *  - stripHtml: декодування HTML entities
 *
 * НОВІ ПОКРАЩЕННЯ v6:
 *  - scan_window_days підтримує обидва поля: scan_window_days та window_days
 *  - gf_scan_logs: зберігає до 200 записів per source (автоочистка старих)
 *  - diagLog: логує всі мережеві помилки з кодом/типом
 *  - scanSingle: повертає diag у відповідь scanSource HTTP endpoint
 *  - rejectDetected: приймає масив detectedIds
 *  - scanAll: паралелізм 3 джерела одночасно (швидше)
 */
...
// ══════════════════════════════════════════════════════════════
async function parseRSS(url, limit, windowDays, isGoogleNews) {
...
// ══════════════════════════════════════════════════════════════
// АВТООЧИСТКА СТАРИХ ЛОГІВ (тримаємо max 200 на джерело)
// ══════════════════════════════════════════════════════════════
...
async function scanSingle(sourceId, src, maxNew) {
  ...
  try {
    if (parser === 'rss' || isGoogleNews) {
      raw = await parseRSS(url, 60, windowDays, isGoogleNews);
    ...
    diagLog('parsed', {
      ...
      sample_titles: raw.slice(0,3).map(i => (i.title||'').slice(0,40))
    });

    // RSS з 0 результатів — спробуємо ширше вікно 14 днів
    if (raw.length === 0 && (parser==='rss'||isGoogleNews)) {
      const raw2 = await parseRSS(url, 60, 14, isGoogleNews);
      if (raw2.length > 0) {
        raw = raw2;
  ...
  const good = raw.filter(item => {
    const pass = passesFilter(item.title, item.description, src);
  ...
  for (const item of good) {
    ...
    // ── Дедуп по заголовку (тільки без URL) ──
    if (norm && !dUrl) {
  ...
  try {
    ...
    // Автоочистка старих логів (не блокуємо)
