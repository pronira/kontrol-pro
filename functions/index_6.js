...
async function parseTelegram(url, limit, windowDays) {
  ...
  $('.tgme_widget_message_wrap').each(function() {
    ...
    if (parts.length > 1) {
      // Мульти-грант допис: призначаємо посилання кожному блоку
      ...
      // Одиночний допис — заголовок з першого <b> або першого рядка
...
async function parsePageLinks(url, limit, src) {
  ...
  for (let attempt = 0; attempt < UA_LIST.length; attempt++) {
    ...
    const timer = setTimeout(() => controller.abort(), 30000);
    try {
      resp = await fetch(url, { headers:{'User-Agent': UA_LIST[attempt]}, signal: controller.signal });
    } finally { clearTimeout(timer); }
