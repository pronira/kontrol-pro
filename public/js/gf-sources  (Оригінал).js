  ...
  if(!current.length){
    ...
    current.forEach(function(s){
      ...
      listH+='<div class="gf-item"><div class="gf-item-head">'
        ...
        +(s.last_checked_at?'<span>'+gfE((s.last_checked_at||'').slice(11,16))+'</span>':'')
        +gfSrcLogBadge(s)
      ...
      if(view==='archive'){
        ...
        listH+='<div class="gf-item-acts">'
          ...
          +'<button class="gf-btn sm g" title="Примусове сканування" onclick="gfForceScan(\''+gfE(s._id||s.source_id)+'\',\''+gfE(s.source_name||'')+'\')">▶ Сканувати</button>'
          +'<button class="gf-btn sm o" title="'+(s.source_status==='active'?'Призупинити':'Відновити')+'" onclick="gfTogglePause(\''+gfE(s._id||s.source_id)+'\')">'+(s.source_status==='active'?'⏸':'▶')+'</button>'
...
async function gfShowScanLog(sourceId, sourceName) {
  ...
  modal.innerHTML = '<div style="width:min(800px,96vw);max-height:90vh;overflow-y:auto;background:#0f172a;border:1px solid rgba(255,255,255,.15);border-radius:14px;padding:22px;color:#e2e8f0;font-family:Geologica,sans-serif">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">'
    + '<h3 style="margin:0;font-size:15px">📋 Лог сканування: '+gfE(sourceName)+'</h3>'
    + '<button onclick="this.parentNode.parentNode.parentNode.remove()" style="background:#ef4444;border:none;color:#fff;border-radius:8px;padding:5px 12px;cursor:pointer;font-weight:700">✕</button>'
    + '</div><div id="gfLogBody" style="font-size:12px">Завантаження…</div></div>';
  document.body.appendChild(modal);

  try {
    // Завантажуємо останні 30 записів лога для цього джерела
    ...
    if (snap.empty) {
      // Показуємо last_scan_log з самого джерела якщо є
      ...
      if (!ll) { body.innerHTML = '<div style="color:#64748b;text-align:center;padding:20px">Лог ще порожній. Дані з’являться після наступного сканування.</div>'; return; }
      snap = { docs: [{ data: function(){ return ll; } }] };
    }

    var rows = '';
    snap.docs.forEach(function(doc) {
      var l = doc.data ? doc.data() : doc;
      var st = l.status || '';
      ...
      if (l.diag_steps && l.diag_steps.length) {
        diagHtml += '<div style="margin-top:10px;border-top:1px solid rgba(255,255,255,.08);padding-top:8px">'
          + '<div style="font-size:10px;font-weight:700;color:#64748b;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px">Діагностика</div>';
        l.diag_steps.forEach(function(step) {
          var stepColor = step.step==='parse_error'||step.step==='multi_grant_error'?'#ef4444':step.step==='result'?'#10b981':'#94a3b8';
          var stepData = Object.assign({}, step);
          delete stepData.step; delete stepData.ts;
          diagHtml += '<div style="display:flex;gap:8px;margin-bottom:4px;font-size:11px">'
            + '<span style="color:#4f6ef7;min-width:110px;flex-shrink:0">['+gfE(step.ts||'')+'] '+gfE(step.step)+'</span>'
            + '<span style="color:'+stepColor+';word-break:break-all">'+gfE(JSON.stringify(stepData).slice(0,200))+'</span>'
            + '</div>';
        });
        diagHtml += '</div>';
      }
      // Попередження/дублі
      var warnHtml = '';
      if (l.diag_warnings && l.diag_warnings.length) {
        warnHtml = '<div style="margin-top:6px;padding:6px 8px;background:rgba(245,158,11,.08);border-radius:6px">'
          + '<div style="font-size:10px;font-weight:700;color:#f59e0b;margin-bottom:4px">Попередження ('+l.diag_warnings.length+')</div>'
          + l.diag_warnings.slice(0,10).map(function(w){return '<div style="font-size:10px;color:#94a3b8;word-break:break-all">'+gfE(w)+'</div>';}).join('')
          + '</div>';
      }
      ...
      rows += '<div style="border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:12px;margin-bottom:8px;background:rgba(255,255,255,.02)">'
        ...
        + (l.detailed?'<span style="background:rgba(255,255,255,.06);padding:2px 8px;border-radius:4px">Деталі: '+l.detailed+' стор</span>':'')
    ...
    var body = document.getElementById('gfLogBody');
    if (body) body.innerHTML = '<div style="color:#ef4444;padding:12px">Помилка: '+gfE(e.message)+'<br><br>'
      + '<small style="color:#94a3b8">Можливо потрібно додати індекс Firestore для колекції gf_scan_logs.<br>'
      + 'Поля: source_id (asc) + scanned_at (desc)</small></div>';
  }
}

function gfLogCell(label, val, color) {
  return '<div style="background:rgba(255,255,255,.04);border-radius:6px;padding:8px;text-align:center">'
    + '<div style="font-size:18px;font-weight:800;color:'+color+'">'+val+'</div>'
    + '<div style="font-size:10px;color:#64748b;margin-top:2px">'+label+'</div></div>';
}

/* ══════ ПОШУК НОВИХ ДЖЕРЕЛ ══════ */
function gfViewDiscover(tabsH, existingSrc) {
  var existingUrls = new Set(existingSrc.map(function(s){ return (s.source_url||'').toLowerCase().replace(/\/+$/,''); }));
  var dismissed = JSON.parse(localStorage.getItem('gf_discover_dismissed')||'[]');
  var dismissedSet = new Set(dismissed);

  // Збираємо всі зовнішні посилання з виявлених грантів
  ...
  det.forEach(function(d) {
    ...
    // Telegram
    var src = (d.source_url||'').toLowerCase();
    if (src.includes('t.me')) {
      var m = src.match(/t\.me\/s?\/?([^\/\?]+)/);
      if (m) {
        var ch = 'telegram:' + m[1];
  ...
  var candidates = Object.keys(urlCount)
    .filter(function(h) {
      ...
      // Фільтруємо якщо вже є як джерело
      var isSrc = existingSrc.some(function(s) {
  ...
  var h = '<div class="gf-panel"><div class="gf-panel-h"><h3>🔎 Пошук нових джерел</h3>'
    + '<span class="gf-badge blue">'+candidates.length+' кандидатів</span></div>'
    + tabsH
    + '<div class="gf-ok" style="margin-bottom:14px">Ці сайти та канали найчастіше зустрічаються у виявлених грантах. Вже додані як джерела — не показуються.</div>';

  if (!candidates.length) {
    return h + '<div class="gf-empty">Немає нових кандидатів. Накопичіть більше виявлених записів.</div></div>';
  }
  ...
  candidates.forEach(function(host) {
    ...
    h += '<div class="gf-item" style="padding:10px 16px">'
      ...
      + (titles.length ? '<div style="font-size:11px;color:#64748b;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+titles.map(function(t){return gfE(t.slice(0,60));}).join(' · ')+'</div>' : '')
      + '</div>'
  ...
  if (dismissed.length) {
    h += '<div style="margin-top:16px;border-top:1px solid rgba(255,255,255,.08);padding-top:14px">'
      + '<div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:8px;text-transform:uppercase;letter-spacing:.4px">Вилучені ('+dismissed.length+')</div>'
    ...
    dismissed.forEach(function(dhost) {
      ...
      var dch = disTg ? dhost.replace('telegram:','') : '';
      var dico = disTg ? '📱' : '🌐';
      var dLabel = disTg ? '@'+dch : dhost;
      h += '<div class="gf-item" style="padding:7px 14px;opacity:.5;display:flex;align-items:center;gap:8px">'
        ...
        + '<div style="flex:1;font-size:12px;color:#64748b">'+dico+' '+gfE(dLabel)+'</div>'
...
function gfDiscoverDismiss(host) {
  var dismissed = JSON.parse(localStorage.getItem('gf_discover_dismissed')||'[]');
  if (dismissed.indexOf(host) < 0) dismissed.push(host);
  localStorage.setItem('gf_discover_dismissed', JSON.stringify(dismissed));
  gfRender();
}
...
async function gfDiscoverAdd(host, url, suggestName) {
  ...
  try {
    ...
    await gfRefresh();
    // Знаходимо щойно збережене джерело і відкриваємо форму
    var newSrc = (GF.data.sources||[]).find(function(s){
      return (s.source_url||'').toLowerCase().replace(/\/+$/, '') === url.toLowerCase().replace(/\/+$/, '');
    });
    if (newSrc) {
      setTimeout(function(){ gfOpenSourceForm(newSrc._id||newSrc.source_id); }, 300);
    }
...
function gfSrcLogBadge(s) {
  ...
  var st = ll.status || '';
  if (!st) return '';
  var color = st==='ok_new'?'var(--green)':st==='ok_dupes'?'var(--accent)':st==='filtered'?'var(--yellow)':st==='error'?'var(--red)':'var(--muted)';
  var icon = st==='ok_new'?'✅ +'+ll.created:st==='ok_dupes'?'♻️ дублі':st==='filtered'?'⚠️ 0':st==='error'?'❌':st==='empty'?'📭':'—';
  return '<span style="color:'+color+'">'+icon+'</span>';
}

/* ── Примусове сканування джерела ── */
async function gfForceScan(sourceId, sourceName) {
  var btn = document.querySelector('[onclick*="gfForceScan(\''+sourceId+'"]');

  // Показуємо індикатор
  gfToast('🔄 Сканую: ' + sourceName + '…', '#4f6ef7');

  // Оновлюємо кнопку якщо знайдена
  var allBtns = document.querySelectorAll('.gf-btn');
  var scanBtn = null;
  allBtns.forEach(function(b) {
    if (b.getAttribute('onclick') && b.getAttribute('onclick').indexOf('gfForceScan') >= 0 && b.getAttribute('onclick').indexOf(sourceId) >= 0) {
      scanBtn = b;
    }
  });
  ...
  try {
    // Викликаємо Cloud Function scanSource
    ...
    if (scanBtn) { scanBtn.disabled = false; scanBtn.textContent = '▶ Сканувати'; }
  }
}
...
function gfWireSources(){
  ...
  content.addEventListener('click', function(e) {
    ...
    if (btn.dataset.discRestore) { localStorage.removeItem('gf_discover_dismissed'); gfRender(); return; }
  }, { once: true });
}
