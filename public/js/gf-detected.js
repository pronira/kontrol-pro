/* ═══════════════════════════════════════════════════════════
   gf-detected.js v2 — Екран «Виявлено» з фільтрами
   Зміни: tooltip на всіх кнопках, "Не підходить" → gfReject(),
          "Відхилені" не відображаються у вкладках крім rejected
   ═══════════════════════════════════════════════════════════ */
...
function gfViewDetected(){
  ...
  var filters = {
    ...
    rejected:   function(d){ return REJECTED.indexOf(d.status) >= 0; },
    all:        function(d){ return REJECTED.indexOf(d.status) < 0; }  // "Усі" — без відхилених
  };

  var counts = {};
  Object.keys(filters).forEach(function(k){ counts[k] = all.filter(filters[k]).length; });
  // "all" рахуємо окремо (з відхиленими)
  counts.allTotal = all.length;
  ...
  var tabs = [
    ['new','Нові',counts.new],
    ['helpful','Корисні',counts.helpful],
    ['review','На ознайомленні',counts.review],
    ['base','У базі',counts.base],
    ['duplicates','Дублікати',counts.duplicates],
    ['rejected','Відхилені',counts.rejected],
    ['all','Усі (без відхилених)',counts.all]
  ];
  ...
  var sh = '<div class="gf-search">'
    ...
    + '<button class="gf-btn sm" title="Пошук" onclick="gfDetDoSearch()">Шукати</button>'
    + (q ? '<button class="gf-btn sm o" title="Скинути пошук" onclick="GF.detectedSearch=\'\';gfRender()">✕</button>' : '')
    + '</div>';
  ...
  modes.forEach(function(m){
    vmh += '<button class="gf-btn sm' + (GF.viewMode===m[0]?'':' o') + '" title="Режим: ' + m[1] + '" onclick="GF.viewMode=\'' + m[0] + '\';gfRender()">' + m[1] + '</button>';
  });
  vmh += '</div>';
  ...
  var bulkH = '<div id="gfBulkBar" style="display:none;margin-bottom:10px;padding:10px 14px;background:var(--accent-soft);border:1px solid rgba(79,110,247,.2);border-radius:var(--r);align-items:center;gap:10px;flex-wrap:wrap">'
    ...
    + '<button class="gf-btn sm g" title="Позначити вибрані як корисні" onclick="gfBulkAction(\'Корисне\')">✓ Корисне</button>'
    + '<button class="gf-btn sm r" title="Відхилити вибрані" onclick="gfBulkAction(\'Не підходить\')">✕ Не підходить</button>'
    + '<button class="gf-btn sm o" title="Скинути вибір" onclick="GF_BULK={};gfUpdateBulkBar();gfRender()">Скинути</button>'
...
function gfDetList(items){
  ...
  items.forEach(function(d){
    ...
    h += '<div class="gf-item" data-det-id="' + gfE(did) + '" style="' + bdr + 'padding:0;overflow:hidden">'
    ...
    h += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;border-top:1px solid var(--border)">'
      ...
      + '<div style="padding:8px 16px;border-right:1px solid var(--border)"><div class="gf-muted" style="font-size:10px">Крайня дата</div><div style="font-size:12px;margin-top:2px">' + (d.deadline ? gfDeadlineBadge(d.deadline)+' '+gfE(d.deadline) : '—') + '</div></div>'
    ...
    h += '<div style="padding:8px 16px;border-top:1px solid var(--border);display:flex;gap:5px;flex-wrap:wrap;background:rgba(255,255,255,.02)">'
      + '<input type="checkbox" class="gf-bulk-chk" data-id="' + gfE(did) + '" id="gfchk_' + gfE(did) + '" onclick="gfToggleBulk(\'' + gfE(did) + '\')" ' + (GF_BULK[did]?' checked':'') + ' style="width:16px;height:16px;cursor:pointer" title="Вибрати для групової дії">'
      + (url ? '<button class="gf-btn sm o" title="Відкрити у новій вкладці" onclick="window.open(\'' + gfE(url) + '\',\'_blank\')">Відкрити</button>' : '')
...
function gfDetCompact(items){
  ...
  items.forEach(function(d){
    ...
    var isPrio = (d._ps||0) > 0;
    var bdr = isPrio ? 'border-left:4px solid var(--accent);' : '';
    h += '<div class="gf-item" data-det-id="' + gfE(did) + '" style="' + bdr + 'padding:8px 14px;display:flex;align-items:center;gap:10px">'
      ...
      + (d.donor ? '<span>' + gfE(d.donor) + '</span>' : '')
      + (d.deadline ? '<span style="color:var(--red)">' + gfE(d.deadline) + '</span>' : '')
      + (d.amount_text ? '<span>' + gfE(d.amount_text) + '</span>' : '')
      + '</div></div>'
      ...
      + '<button class="gf-btn sm g" title="Корисне" onclick="gfDetStatus(\'' + gfE(did) + '\',\'Корисне\')">✓</button>'
      + '<button class="gf-btn sm r" title="Не підходить" onclick="gfOpenStatusModal(\'' + gfE(did) + '\',\'Не підходить\')">✕</button>'
      + '</div></div>';
  });
  return h + '</div>';
}
...
function gfStatusBadge(st){
  ...
  else if (/не підходить|відхилено|видалено/i.test(s)) c = 'red';
...
async function gfDetStatus(id, status){
  try {
    ...
    gfRender();
    await gfSetDetectedStatus(id, status, '', '');
    gfToast(status==='Корисне' ? '✓ Позначено корисним' : '✕ Статус оновлено', status==='Корисне' ? 'var(--green)' : 'var(--red)');
  } catch(e) { alert('Помилка: ' + e.message); await gfRefresh(); }
}

function gfWireDetected(){ /* handled inline */ }
